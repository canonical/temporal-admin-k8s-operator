...
"""Define the Postgresql relation."""
...
class Postgresql(framework.Object):
    ...
    def _on_database_changed(self, event) -> None:
        ...
        if self.charm._state.database_connections_from_db is None:
            self.charm._state.database_connections_from_db = {"db": None, "visibility": None}
        ...
        self.charm._setup_db_schemas(event)
    ...
    def _update_db_connections(self, rel_name, db_conn):
        ...
        """
        if self.charm._state.database_connections_from_db is None:
            self.charm._state.database_connections_from_db = {}

        database_connections = self.charm._state.database_connections_from_db
        database_connections[rel_name] = db_conn
        self.charm._state.database_connections_from_db = database_connections
